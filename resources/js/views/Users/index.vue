        ...
        <div class="col-12">
            <div class="table-responsive bg-white border border-secondary shadow-sm rounded-3 p-3">
                ...
                <table class="table">
                    ...
                    <tbody>
                        <tr v-for="(item, index) in filteredUsers" class="align-middle">
                            <td>{{ index + 1 }}</td>
                            <td>{{ item.name }}</td>
                            <td>{{ item.email }}</td>
                            ...
                            <td> <span v-if="item.pendingUpdate">
                                    ...
                                    <button class="btn btn-sm btn-danger"
                                        @click="item.pendingUpdate = false">Cancel</button> </span>
                                <span v-else>
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuLink">
                                        ...
                                        <li><a class="dropdown-item" @click="getDetails(item.id)">Edit</a>
                                        </li>
...
const filteredUsers = computed(() => {
    return users.items.filter((users) => {
        return users.name.toLowerCase().includes(searchQuery.value.toLowerCase())
    })
})
